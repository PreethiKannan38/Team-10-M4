...
function CanvasWorkspace({ canvasEngineRef }) {
  ...
  const handleAction = (actionId) => {
    ...
    switch (actionId) {
      case 'undo': canvasEngineRef.current.undo(); break;
      case 'redo': canvasEngineRef.current.redo(); break;
      case 'clear': canvasEngineRef.current.clearAll(); break;
      case 'export': canvasEngineRef.current.exportToImage(); break;
  ...
  return (
    <div className="w-screen h-screen flex flex-col overflow-hidden font-sans transition-colors duration-300" style={{ background: t.workspaceBg }}>
      ...
      <ChatPanel engine={canvasEngineRef.current} currentUser={currentUser} isOpen={isChatOpen} onClose={() => setIsChatOpen(false)} />
      <TimelineControls canvasId={canvasId} engine={canvasEngineRef.current} isOpen={isTimelineOpen} onClose={() => setIsTimelineOpen(false)} />
